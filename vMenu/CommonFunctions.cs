...
namespace vMenuClient
{
    public static class CommonFunctions
    {
        ...
        public static async Task<int> SpawnVehicle(uint vehicleHash, bool spawnInside, bool replacePrevious, bool skipLoad, VehicleInfo vehicleInfo, string saveName = null, float x = 0f, float y = 0f, float z = 0f, float heading = -1f)
        {
            ...
            if (!(vehicleInfo.colors["red"] == 1337 || vehicleInfo.colors["green"] == 1337 || vehicleInfo.colors["blue"] == 1337))
            {
                SetVehicleCustomPrimaryColour(vehicle.Handle, vehicleInfo.colors["red"], vehicleInfo.colors["green"], vehicleInfo.colors["blue"]);
            }
            if (!(vehicleInfo.colors["red2"] == 1337 || vehicleInfo.colors["green2"] == 1337 || vehicleInfo.colors["blue2"] == 1337))
            {
                SetVehicleCustomSecondaryColour(vehicle.Handle, vehicleInfo.colors["red2"], vehicleInfo.colors["green2"], vehicleInfo.colors["blue2"]);
            }
        ...
        public static async void SaveVehicle(string updateExistingSavedVehicleName = null)
        {
            ...
            if (Game.PlayerPed.IsInVehicle())
            {
                ...
                if (veh != null && veh.Exists() && !veh.IsDead && veh.IsDriveable)
                {
                    ...
                    int green2 = 1337;
                    int blue2 = 1337;
                    GetVehicleCustomSecondaryColour(veh.Handle, ref red2, ref green2, ref blue2);
                    colors.Add("red2", red2);
                    colors.Add("green2", green2);
                    colors.Add("blue2", blue2);
